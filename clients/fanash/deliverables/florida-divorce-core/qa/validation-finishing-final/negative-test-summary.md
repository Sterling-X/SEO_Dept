...
| A direct Wisconsin client URL may differ from its preserved V2 path | Yes | 0 | `PASS: Supported deterministic Core Hub checks cleared.` |
| Nine sources remain allowed and use the 12 pt Sources contract | Yes | 0 | `PASS: Supported deterministic Core Hub checks cleared.` |
...
| A client mapping cannot change the V2 reference path | Yes | 1 | `v2_reference_path must match V2 exactly` |
| A same-origin client URL cannot be assigned to the wrong jurisdiction | Yes | 1 | `client_jurisdiction must match meta.jurisdiction` |
| A client URL cannot bypass the directional V2 relationship | Yes | 1 | `no directional V2` |
| Four true sentences rejected after citation-aware parsing | Yes | 1 | `exceeds 3 sentences (4)` |
| Sources declared outside first-appearance order are rejected | Yes | 1 | `Sources must be ordered by first in-body citation appearance` |
| Swapped citation hyperlink targets are rejected | Yes | 1 | `In-body citation hyperlinks must appear in [1] through [N] order and target the matching source URL` |
