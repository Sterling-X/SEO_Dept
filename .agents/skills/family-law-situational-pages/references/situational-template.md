...
The first two content blocks must be paragraphs. A body citation must appear once at the first material claim supported by that source; the generator creates the second source URL occurrence in the Sources section.
...
- No em dashes, unresolved placeholders, tag-on link language, or duplicate internal destination URLs.
