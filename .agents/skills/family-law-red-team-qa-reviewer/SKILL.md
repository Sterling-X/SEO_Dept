---
name: family-law-red-team-qa-reviewer
...
# Family Law Red-Team QA Reviewer
...
3. **Intent and structure**
   ...
   - **internal link discipline:**
     - duplicate destination URLs (any URL appearing more than once is a hard fail)
...
## Voice Skill Cross-Reference (Required)

Before reviewing any content, identify the firm's voice skill from the available skills in the current project. There will be one voice skill present. It is the authoritative reference for tone, persona, messaging pillars, banned patterns, differentiators, and audience framing.

How to identify the voice skill:
- Scan the available skills list for any skill with "voice" in the name (e.g., `[firm]-voice`)
- Load that skill's SKILL.md before proceeding to review
- If no voice skill is found in the project, flag brand voice alignment as a residual unknown and note that no authoritative voice reference exists for the review

Do not evaluate brand voice from memory or general impressions. The voice skill is the source of truth. Do not maintain or reference a static list of voice skills. Identify the voice skill dynamically every time QA is requested.
...
1b. **Identify and load the project's voice skill**
   - scan available skills for the voice skill (any skill with "voice" in the name)
   - read its SKILL.md before proceeding to review
   - if no voice skill is found, note this limitation and proceed without brand voice scoring
...
- approving a page where the same destination URL appears more than once (hard fail)
