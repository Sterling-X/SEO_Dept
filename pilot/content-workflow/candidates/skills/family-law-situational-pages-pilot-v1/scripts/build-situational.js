#!/usr/bin/env node
/** PILOT CANDIDATE v1 of the FL-M008 local replacement generator. Baseline: .agents/skills/family-law-situational-pages/scripts/build-situational.js (hash in ../pilot-manifest.json). Changes: repository-root discovery, broader placeholder grammar, metadata placeholder check. */
/**
...
// PILOT CANDIDATE v1: locate the repository root by marker files instead of a fixed depth,
// so the candidate works from pilot/content-workflow/candidates/skills/ as well as .agents/skills/.
function findRepoRoot(start) {
  if (process.env.SEO_DEPT_ROOT) return path.resolve(process.env.SEO_DEPT_ROOT);
  let current = start;
  for (let depth = 0; depth < 12; depth += 1) {
    if (fs.existsSync(path.join(current, "AGENTS.md"))
        && fs.existsSync(path.join(current, "context/architecture/Family_Law_StructureV2.html"))) return current;
    const parent = path.dirname(current);
    if (parent === current) break;
    current = parent;
  }
  fail([`Cannot locate the repository root (AGENTS.md plus governing V2 HTML) above ${start}.`]);
  return start;
}
...
const consumerPlaceholder = findPlaceholder(consumerText);
if (consumerPlaceholder) errors.push(`Consumer copy contains an unresolved placeholder: ${consumerPlaceholder}`);
...
const metaPlaceholder = findPlaceholder([meta.title_tag, meta.meta_description, meta.cms_title, meta.social_title, meta.social_description, meta.h1, meta.media_note].join(" "));
if (metaPlaceholder) errors.push(`Publisher metadata contains an unresolved placeholder: ${metaPlaceholder}`);
if (errors.length) fail(errors);
