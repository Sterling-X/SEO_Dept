...
def production_shaped_manifest() -> dict:
    """Synthetic content in production mode so that sources and citation runs are allowed.

    Every value is a labeled test fixture; the generator checks evidence shape, not truth.
    """
    manifest = synthetic_manifest(False)
    manifest["meta"].pop("synthetic_fixture", None)
    manifest["meta"].update({
        "firm_name": "Synthetic Test Firm",
        "jurisdiction": "Synthetic Jurisdiction",
        "voice_source": "SYNTHETIC TEST FIXTURE voice brief",
        "client_url_status": 200,
        "client_url_redirects": 0,
        "client_url_verified_on": "2026-09-23",
        "client_url_evidence": "SYNTHETIC TEST FIXTURE; no live request was made.",
    })
    manifest["link_inventory"] = {"status": "reviewed", "reviewed_on": "2026-09-23", "evidence": "SYNTHETIC TEST FIXTURE; no live screen was made."}
    manifest["sources"] = [
        {"id": 1, "label": "Synthetic Example Statute 1", "url": "https://example.com/statutes/1", "authority": "official-primary", "status": 200, "verified_on": "2026-09-23", "verification_evidence": "SYNTHETIC TEST FIXTURE"},
        {"id": 2, "label": "Synthetic Example Statute 2", "url": "https://example.com/statutes/2", "authority": "official-primary", "status": 200, "verified_on": "2026-09-23", "verification_evidence": "SYNTHETIC TEST FIXTURE"},
    ]
    stakes_index = next(i for i, b in enumerate(manifest["content"]) if b.get("type") == "h2" and b.get("role") == "stakes")
    manifest["content"][stakes_index + 1]["runs"].extend([{"type": "text", "text": " "}, {"type": "citation", "source_id": 1, "text": "[1]"}])
    strategy_index = next(i for i, b in enumerate(manifest["content"]) if b.get("type") == "h2" and b.get("role") == "strategy")
    manifest["content"][strategy_index + 2]["runs"].extend([{"type": "text", "text": " "}, {"type": "citation", "source_id": 2, "text": "[2]"}])
    return manifest
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="fl-m008-workflow-") as raw_temp:
        ...
        prod_dir.mkdir()
        passed, output, prod_docx, prod_manifest = generate_and_validate(prod_dir, production_shaped_manifest())
        results.append(("positive-production-shaped-with-citations", passed, output))
        if prod_docx.exists():
            def inject(needle: str, replacement: str):
                def transform(payload: bytes) -> bytes:
                    if needle.encode("utf-8") not in payload:
                        raise RuntimeError(f"Injection anchor not found: {needle[:60]!r}")
                    return payload.replace(needle.encode("utf-8"), replacement.encode("utf-8"), 1)
                return transform

            def tampered(name: str, transform, expect_page: str | None, expect_structural: str | None) -> None:
                target = root / name / "synthetic-high-conflict-divorce-situational.docx"
                target.parent.mkdir()
                rewrite_docx(prod_docx, target, "word/document.xml", transform)
                if expect_page is not None:
                    completed = run(["node", str(PAGE), str(target), "--manifest", str(prod_manifest)])
                    results.append((f"negative-{name}-page", completed.returncode != 0 and expect_page in completed.stdout, completed.stdout))
                if expect_structural is not None:
            ...
            tampered("mustache-placeholder", inject(OPENING_TWO, OPENING_TWO + " {{county}}"), "Unresolved placeholder detected: {{county}}", "Unresolved placeholder detected")
            tampered("marker-word-placeholder", inject(OPENING_TWO, OPENING_TWO + " TBD"), "Unresolved placeholder detected: TBD", "Unresolved placeholder detected")
    ...
    if failed:
        ...
        return 1
    print(f"PASS: {len(results)} FL-M008 local workflow regression cases (pilot candidate v1).")
    return 0
