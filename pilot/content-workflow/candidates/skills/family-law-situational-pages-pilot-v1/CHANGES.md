...
|---|---|---|---|
| 1 | Conflicting citation-placement and duplicate-URL requirements | `SKILL.md` "In-body format" describes an identifier followed by a bracketed number while the template and generator emit a hyperlinked `[n]` run and the page validator requires the anchor to be exactly `[n]`; the red-team skill calls any repeated URL a hard fail while the situational skill requires the statute pairing. | One format stated in `SKILL.md`, the template, and `canonical/link-and-cta-limits.md`: hyperlinked `[n]` at first material claim, first-appearance numbering, each source once, Sources last as `[n] label \| URL`; pairing declared exempt from the single-placement rule; every other URL exactly once. |
...
- The render step still delegates to the pinned Core renderer, which fails closed on this
  machine (renderer release mismatch recorded in `CLAUDE.md`). The candidate does not change
  that; rendered-page inspection remains unavailable until the lock is updated.
...
- The bracket-token placeholder rule will flag legitimate bracketed prose such as "[sic]".
  ...
  silently.
- Whether "cite once at first mention" should permit a second hyperlinked reference on very
  long pages is unresolved; the pilot enforces once.
