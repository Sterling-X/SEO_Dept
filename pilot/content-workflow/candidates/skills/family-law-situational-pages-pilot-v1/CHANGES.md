...
| 8 | Placeholder-first instructions ("leave a placeholder rather than guessing"; "use a clear placeholder" for hyper-local details) let a page finish with unresearched facts, and nothing tied a citation to an actual retrieval. | Research-first (2026-09-24): every authority and changeable client fact is retrieved during the run with `scripts/research_fetch.py` and named by the pre-draft row; a `[LOCAL DETAIL]` marker is a reporting device for a fact confirmed unobtainable now; Quality Gates 7 and 10 require the research record. Enforced by `readiness_check.py` (`RESEARCH_*`, `LEGISLATION_NOT_EFFECTIVE`, `LEGAL_LOG_NO_EVIDENCE`) and `record_review.py`. | `### State-Specific Content: Research and Verification`, Quality Gates 7 and 10, template placeholder rule, amendment summary item 8 |
