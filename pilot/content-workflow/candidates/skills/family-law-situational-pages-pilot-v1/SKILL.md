---
name: family-law-situational-pages-pilot-v1
...
metadata:
  ...
  baseline: .agents/skills/family-law-situational-pages
  changes: CHANGES.md
---

# Family Law Situational Page Writer (pilot candidate v1)

> **PILOT CANDIDATE / NOT PRODUCTION DEFAULT.** Derived from the baseline skill at
> `.agents/skills/family-law-situational-pages` (baseline hashes in `pilot-manifest.json`). Every
> change is listed in `CHANGES.md`. A content-workflow run pins this directory's `SKILL.md` by path
> and SHA-256; `pilot-manifest.json` lists the resources that must exist before the run may start.
...
3. **Voice by exact domain route.** `pilot/content-workflow/canonical/client-routing.json` (mirror of
   `AGENTS.md`) names the voice skill; an unrouted domain needs an approved voice brief pinned in the
   run; otherwise the run is INCOMPLETE. No name-pattern scanning, no vertical inference.
4. **Placeholders block delivery.** Only `[LOCAL DETAIL: ...]` is a valid placeholder form, and any
   remaining placeholder makes the run INCOMPLETE. Validators now reject every bracketed non-numeric
   token, `{{...}}`, `<<...>>`, `TODO`, `TBD`, `FIXME`, `XXX`, `TK`, and `lorem ipsum`.
5. **Validators reject citation mismatches.** Plain-text markers, unknown marker numbers, out-of-order
   markers, and Sources rows whose id, label, or URL differ from the manifest now fail.
6. **Required resources are declared.** `pilot-manifest.json` lists template, scripts, and installed
...
7. **Brand voice** -- how the firm sounds. This is not optional, and this skill does not ask the user to describe the voice in free text if a canonical voice skill exists for the client.
   ...
   **Voice source is a hard prerequisite, resolved by exact domain route.** Look up the client's
   domain in `pilot/content-workflow/canonical/client-routing.json` (a mirror of the `AGENTS.md`
   routing table). A routed domain names exactly one voice skill: read its full SKILL.md before
   drafting; it is canonical, and session direction supplements but does not override it (flag
   conflicts). Never select a voice skill by scanning names for `-voice`, and never infer one from the
   vertical; this repository holds several voice skills and a name scan can pick another client's.

   **If the domain is unrouted:** the run must pin an approved, dated voice brief as a source
   (`kind: voice-brief`). Read it in full. If neither a route nor a pinned brief exists, stop and
   return status INCOMPLETE; do not draft with a default voice.
...
1. **Confirm the routed voice source is loaded.** Before reading the template or drafting any content,
   verify that the voice skill named by the exact domain route, or the approved voice brief pinned for
   an unrouted domain, has been read in full. If neither exists, stop and return INCOMPLETE.
...
**Before writing anything, internalize the routed voice source.** The voice skill named by the exact domain route, or the approved voice brief pinned for an unrouted domain, was loaded at session start as a hard prerequisite. Read it, understand it, and write through it. If the firm is direct and no-nonsense, write that way. If the firm is warm and reassuring, write that way. If the firm leads with confidence and aggression, write that way. The voice should be consistent from the first paragraph to the last.

**Voice source authority:** The routed voice source is canonical. It defines positioning, tone, cadence, vocabulary, phrases to use, phrases to avoid, and brand guardrails. User-provided voice direction for this session supplements but does not override. If session direction conflicts with the voice skill, default to the voice skill.
...
**If no routed voice skill and no pinned approved voice brief exist:** do not draft. Return INCOMPLETE naming the missing voice route. A default voice is not a substitute inside a content-workflow run.
...
Counts and placements below are restated from the single pilot table
`pilot/content-workflow/canonical/link-and-cta-limits.md`; if the two ever differ, the table governs.

- **Consultation CTA:** exactly one hyperlinked consultation CTA in the final CTA section, before Sources. At most one non-linked CTA sentence may appear earlier; it makes no guarantee, pricing, urgency, or "free" claim unless the approved client facts support it.
...
After generating the DOCX, run both scans from this candidate's directory
(`pilot/content-workflow/candidates/skills/family-law-situational-pages-pilot-v1/`) before handing the
draft to the reviewers:

1. `python3 scripts/office/validate.py path/to/page.docx --manifest path/to/workflow-input.json` for structural validation (now including placeholder and citation-marker checks).
2. `node scripts/validate-page.js path/to/page.docx --manifest path/to/workflow-input.json` for the page-level pre-delivery scan (now including plain-text marker, marker order, and Sources-row parity checks).

Inside a content-workflow run, also declare both commands under `export.validators` in `run.json` so
`scripts/mechanical_qa.py` and `scripts/deliver.py` execute them against the exported file and record
the exit codes. A validator PASS is mechanical evidence only.
...
who raised it decides that on recheck after re-reading the revised passage. Any edit after a review
changes the draft hash and invalidates that review record. At most two repair rounds follow the
